...
import { CentralPlanEdit } from "./central-plan-edit";



describe("central plan edit tests", () => {
    it("loadPlan calls expectedservice and sets initial data", async () => {
      // Arrange
      const dto: CentralPlanViewModelDto = getCentralPlanViewModelDto();
      const svc = new CentralPlanReportService();
      spyOn(svc, "getPlan").and.returnValue(Promise.resolve(dto));
      const sut = new CentralPlanEdit(svc, new SignalrWrapper(), undefined);
      sut.planid = 2;
    ...
    it("submitPlan calls expected service", async () => {
      ...
      spyOn(centralPlanReportService, "submitPlan").and.returnValue(Promise.resolve());
      const sut = new CentralPlanEdit(centralPlanReportService, new SignalrWrapper(), undefined);
      sut.planid = 13;
      sut.centralPlan = dto;
      ...
      expect(centralPlanReportService.submitPlan).toHaveBeenCalledWith(2, 13);
  });
  it("save does nothin when not dirty", async () => {
    // Arrange
    const dto: CentralPlanViewModelDto = getCentralPlanViewModelDto();
    const centralPlanReportService = new CentralPlanReportService();
    spyOn(centralPlanReportService, "updatePlan").and.returnValue(Promise.resolve());
    const sut = new CentralPlanEdit(centralPlanReportService, new SignalrWrapper(), undefined);
    sut.planid = 1007;
    sut.centralPlan = dto;
    sut.setInitialData();
    ...
    expect(centralPlanReportService.updatePlan).not.toHaveBeenCalled();
  ...
  it("save calls expected service when dirty", async () => {
    ...
    //Act
    sut.centralPlan.preliminaryMemberPlanData.upgradeTarget = 45;
    await sut.save();

    //Assert
    const expectedPlanData = getExpectedPlanData(sut.centralPlan);
    expect(sut.isSaving).toBe(true);
    expect(centralPlanReportService.updatePlan).toHaveBeenCalledWith(2, 1007, expectedPlanData);
    expect(sut.afterSave).toEqual([]);
  });
  ...
  it("saveAndSubmit calls expected service when dirty and set expected afterSave", async () => {
    ...
    //Act
    sut.centralPlan.preliminaryMemberPlanData.upgradeTarget = 67;
  ...
  it("onCentralPlanUpdated sets flag and calls afterSave", async () => {
    // Arrange
    const dto: CentralPlanViewModelDto = getCentralPlanViewModelDto();
    const svc = new CentralPlanReportService();
    ...
    // Act
    await sut.onCentralPlanUpdated(1007);
  ...
  it("onCentralPlanUpdated sets flag and does nothing if no afterSave", async () => {
    // Arrange
    const dto: CentralPlanViewModelDto = getCentralPlanViewModelDto();
    const svc = new CentralPlanReportService();
    spyOn(svc, "submitPlan").and.returnValue(Promise.resolve());
  ...
  it("onCentralPlanUpdateFailed sets flag and does nothing if no afterSave", () => {
    // Arrange
    const dto: CentralPlanViewModelDto = getCentralPlanViewModelDto();
    const svc = new CentralPlanReportService();
    spyOn(svc, "submitPlan").and.returnValue(Promise.resolve());
    ...
    // Act
    sut.onCentralPlanUpdateFailed({$values: ["error"]});
  ...
  it("onCentralPlanUpdateFailed sets flag and clears afterSave", () => {
    // Arrange
    const dto: CentralPlanViewModelDto = getCentralPlanViewModelDto();
    const svc = new CentralPlanReportService();
    spyOn(svc, "submitPlan").and.returnValue(Promise.resolve());
  ...
  function getCentralPlanViewModelDto(): CentralPlanViewModelDto {
    return {
  ...
  function getExpectedPlanData(dto: CentralPlanViewModelDto): PlanData {
    return new PlanData(
