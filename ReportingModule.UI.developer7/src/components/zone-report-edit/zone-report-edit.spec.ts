...
describe("zone report edit tests", () => {
    ...
    it("save calls expected service and sets flag", async () => {
        ...
        sut.reportid = 11;
        sut.zoneReport = dto;
        sut.zoneReport.associateMemberData.increased = 45;
    ...
    it("cancelChanges calls expected service", async () => {
        const svc = new ZonePlanReportService();
        spyOn(svc, "getReport").and.returnValue(Promise.resolve(getZoneReportDto()));
        const sut = new ZoneReportEdit(svc, undefined, undefined);
        sut.reportid = 11;
        sut.zoneReport = getZoneReportDto();
        sut.zoneReport.associateMemberData.decreased = 5;

        await sut.cancelChanges();

        expect(svc.getReport).toHaveBeenCalledWith(11);

        expect(sut.zoneReport).toEqual(getZoneReportDto());
    });
    ...
    it("onzoneReportUpdated updates flag, calls expected service and does not call submit report when no afterSave", async () => {
        const svc = new ZonePlanReportService();
        spyOn(svc, "getReport").and.returnValue(Promise.resolve(getZoneReportDto()));
        const sut = new ZoneReportEdit(svc, undefined, undefined);
        sut.reportid = 11;
        ...
        await sut.onZoneReportUpdated(11);
    ...
    it("onzoneReportUpdated updates flag, calls expected service and calls submit report when afterSave", async () => {
        const svc = new ZonePlanReportService();
        spyOn(svc, "getReport").and.returnValue(Promise.resolve(getZoneReportDto()));
    ...
    it("onZoneReportUpdateFailed clears flag and does nothing if no afterSave", () => {
        const sut = new ZoneReportEdit(new ZonePlanReportService(), undefined, undefined);
        sut.isSaving = true;
    ...
    it("onZoneReportUpdateFailed clears flag and pops afterSave", () => {
        const sut = new ZoneReportEdit(new ZonePlanReportService(), undefined, undefined);
        sut.isSaving = true;
        sut.afterSave.push(sut.submitReport);
    ...
    it("onReportSubmitted clears flag and routes to expected view", () => {
        ...
        const sut = new ZoneReportEdit(undefined, undefined, router);
        ...
        expect(sut.isSubmitting).toBe(false);
        expect(router.navigate).toHaveBeenCalledWith("zone-report-view/11");
    });
...
function getExpectedReportData(data: ZoneReportViewModelDto): ReportData {
