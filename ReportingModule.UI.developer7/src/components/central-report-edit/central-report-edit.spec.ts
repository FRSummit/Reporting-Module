...
import { CentralReportEdit } from "./central-report-edit";
import { CentralReportViewModelDto } from "models/CentralReportViewModelDto";

describe("central report edit tests", () => {
    it("loadReport calls expected service", async () => {
        const dto = getCentralReportDto();
        const svc = new CentralPlanReportService();
        spyOn(svc, "getReport").and.returnValue(Promise.resolve(dto));
        const sut = new CentralReportEdit(svc, undefined, undefined);
        sut.reportid = 11;

        await sut.loadReport();

        expect(svc.getReport).toHaveBeenCalledWith(11);
        expect(sut.centralReport).toEqual(dto);
    ...
    it("save calls expected service and sets flag", async () => {
        ...
        const sut = new CentralReportEdit(svc, undefined, undefined);
        sut.reportid = 11;
        sut.centralReport = dto;
        sut.centralReport.associateMemberData.increased = 45;
        const expectedReportData = getExpectedReportData(sut.centralReport);
    ...
    it("cancelChanges calls expected service", async () => {
        const svc = new CentralPlanReportService();
        spyOn(svc, "getReport").and.returnValue(Promise.resolve(getCentralReportDto()));
        const sut = new CentralReportEdit(svc, undefined, undefined);
        sut.reportid = 11;
        sut.centralReport = getCentralReportDto();
        sut.centralReport.associateMemberData.decreased = 5;

        await sut.cancelChanges();

        expect(svc.getReport).toHaveBeenCalledWith(11);

        expect(sut.centralReport).toEqual(getCentralReportDto());
    });
    ...
    it("oncentralReportUpdated updates flag, calls expected service and does not call submit report when no afterSave", async () => {
        const svc = new CentralPlanReportService();
        spyOn(svc, "getReport").and.returnValue(Promise.resolve(getCentralReportDto()));
        const sut = new CentralReportEdit(svc, undefined, undefined);
        sut.reportid = 11;
        ...
        await sut.onCentralReportUpdated(11);
    ...
    it("oncentralReportUpdated updates flag, calls expected service and calls submit report when afterSave", async () => {
        const svc = new CentralPlanReportService();
        spyOn(svc, "getReport").and.returnValue(Promise.resolve(getCentralReportDto()));
    ...
    it("onCentralReportUpdateFailed clears flag and does nothing if no afterSave", () => {
        const sut = new CentralReportEdit(new CentralPlanReportService(), undefined, undefined);
        sut.isSaving = true;

        sut.onCentralReportUpdateFailed({$values: ["error"]});

        expect(sut.isSaving).toBe(false);
    });

    it("onCentralReportUpdateFailed clears flag and pops afterSave", () => {
        const sut = new CentralReportEdit(new CentralPlanReportService(), undefined, undefined);
        sut.isSaving = true;
        sut.afterSave.push(sut.submitReport);
    ...
    it("onReportSubmitted clears flag and routes to expected view", () => {
        ...
        const sut = new CentralReportEdit(undefined, undefined, router);
        ...
        expect(sut.isSubmitting).toBe(false);
        expect(router.navigate).toHaveBeenCalledWith("central-report-view/11");
    });
...
function getCentralReportDto(): CentralReportViewModelDto {
    return {
...
function getExpectedReportData(data: CentralReportViewModelDto): ReportData {
