...
import { StatePlanEdit } from './state-plan-edit';
...
describe("state plan edit tests", () => {
    ...
    // Arrange
    it("statePlan calls expected service", async () => {
      const dto: StatePlanViewModelDto = getStatePlanViewModelDto();
      
      const statePlanReportService = new StatePlanReportService();
      spyOn(statePlanReportService, "submitPlan").and.returnValue(Promise.resolve());
      const sut = new StatePlanEdit(statePlanReportService, new SignalrWrapper(), undefined);
      sut.planid = 13;
      sut.statePlan = dto;
      ...
      expect(statePlanReportService.submitPlan).toHaveBeenCalledWith(2, 13);
  });
  it("save does nothin when not dirty", async () => {
    // Arrange
    const dto: StatePlanViewModelDto = getStatePlanViewModelDto();
    const statePlanReportService = new StatePlanReportService();
    spyOn(statePlanReportService, "updatePlan").and.returnValue(Promise.resolve());
    const sut = new StatePlanEdit(statePlanReportService, new SignalrWrapper(), undefined);
    sut.planid = 1007;
    sut.statePlan = dto;
    sut.setInitialData();
    ...
    expect(statePlanReportService.updatePlan).not.toHaveBeenCalled();
  ...
  it("save calls expected service when dirty", async () => {
    ...
    //Act
    sut.statePlan.preliminaryMemberPlanData.upgradeTarget = 45;
    await sut.save();

    //Assert
    const expectedPlanData = getExpectedPlanData(sut.statePlan);
    expect(sut.isSaving).toBe(true);
    expect(statePlanReportService.updatePlan).toHaveBeenCalledWith(2, 1007, expectedPlanData);
    expect(sut.afterSave).toEqual([]);
  });
  ...
  it("saveAndSubmit calls expected service when dirty and set expected afterSave", async () => {
    ...
    //Act
    sut.statePlan.preliminaryMemberPlanData.upgradeTarget = 67;
  ...
  it("onStatePlanUpdated sets flag and calls afterSave", async () => {
    // Arrange
    const dto: StatePlanViewModelDto = getStatePlanViewModelDto();
    const svc = new StatePlanReportService();
    ...
    // Act
    await sut.onStatePlanUpdated(1007);
  ...
  it("onStatePlanUpdateFailed sets flag and does nothing if no afterSave", () => {
    // Arrange
    const dto: StatePlanViewModelDto = getStatePlanViewModelDto();
    const svc = new StatePlanReportService();
    spyOn(svc, "submitPlan").and.returnValue(Promise.resolve());
  ...
  it("onStatePlanUpdateFailed sets flag and clears afterSave", () => {
    // Arrange
    const dto: StatePlanViewModelDto = getStatePlanViewModelDto();
    const svc = new StatePlanReportService();
    spyOn(svc, "submitPlan").and.returnValue(Promise.resolve());
  ...
  function getStatePlanViewModelDto(): StatePlanViewModelDto {
    return {
  ...
  function getExpectedPlanData(dto: StatePlanViewModelDto): PlanData {
    return new PlanData(
