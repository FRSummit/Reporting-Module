...
describe("state report lastperiod edit tests", () => {
    ...
    it("save does nothing when not dirty", async () => {
      // Arrange
      const dto: StateReportViewModelDto = getStateReportDto();
      const zonePlanReportService = new StatePlanReportService();
      spyOn(zonePlanReportService, "updateReportLastPeriod").and.returnValue(Promise.resolve());
      const sut = new StateReportLastPeriodEdit(zonePlanReportService, new SignalrWrapper(), undefined);
      sut.reportid = 11;
      sut.stateReport = dto;
      sut.setInitialData();
    ...
    it("save calls expected service when dirty", async () => {
      ...
      sut.stateReport.associateMemberData.increased = 25;
      await sut.save();
  ...
  it("onUnitReportUpdated sets flag", async () => {
    ...
    // Act
    await sut.onStateReportUpdated(3);
  ...
  it("isDirty tracks dto state", async () => {
      const dto: StateReportViewModelDto = getStateReportDto();
      const sut = new StateReportLastPeriodEdit(new StatePlanReportService(), new SignalrWrapper(), undefined);
      sut.reportid = 11;
      sut.stateReport = dto;
      ...
      sut.stateReport.associateMemberData.increased = 42;
      expect(sut.isDirty).toBe(true);
  ...
  it("cancelChanges calls expected service method and resets dirty flag", async () => {
      ...
      sut.setInitialData();
      expect(sut.isDirty).toBe(false);
      sut.stateReport.associateMemberData.increased = 55;
      expect(sut.isDirty).toBe(true);
  ...
  it("onUnitReportUpdated resets saving and calls expected method", () => {
      ...
      sut.stateReport.preliminaryMemberData.decreased = 34;
      sut.isSaving = true;
      expect(sut.isDirty).toBe(true);
      
      sut.onStateReportUpdated(3);
  ...
  it("onUnitReportUpdateFailed only resets saving flag and not dirty flag", () => {
      ...
      sut.onStateReportUpdateFailed({$values: ["error"]});
  ...
  function getStateReportDto(): StateReportViewModelDto {
    return {
      ...
      function getExpectedReportData(dto: StateReportViewModelDto): ReportLastPeriodUpdateData {
