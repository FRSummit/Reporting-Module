...
import { CentralReportLastPeriodEdit } from "./central-report-lastperiod-edit";




describe("state report lastperiod edit tests", () => {
    it("loadReport calls expected service method and sets data", async () => {
        const dto: CentralReportViewModelDto = getCentralReportDto();
        const svc = new CentralPlanReportService();
        spyOn(svc, "getReport").and.returnValue(Promise.resolve(dto));
        const sut = new CentralReportLastPeriodEdit(svc, new SignalrWrapper(), undefined);
        sut.reportid = 11;
        
        await sut.loadReport();

        expect(svc.getReport).toHaveBeenCalledWith(11);
        expect(sut.centralReport).toBe(dto);
    ...
    it("save does nothing when not dirty", async () => {
      ...
      const dto: CentralReportViewModelDto = getCentralReportDto();
      const svc = new CentralPlanReportService();
      spyOn(svc, "updateReportLastPeriod").and.returnValue(Promise.resolve());
      const sut = new CentralReportLastPeriodEdit(svc, new SignalrWrapper(), undefined);
      sut.reportid = 11;
      ...
      expect(svc.updateReportLastPeriod).not.toHaveBeenCalled();
    });
    ...
    it("save calls expected service when dirty", async () => {
      ...
      sut.centralReport.associateMemberData.increased = 25;
      await sut.save();

      const expectedReportData = getExpectedReportData(sut.centralReport);
      expect(sut.isSaving).toBe(true);
      expect(svc.updateReportLastPeriod).toHaveBeenCalledWith(2, 11, expectedReportData);
  ...
  it("onUnitReportUpdated sets flag", async () => {
    ...
    // Act
    await sut.onCentralReportUpdated(3);

    expect(sut.isSaving).toBe(false);
    expect(svc.getReport).toHaveBeenCalled();
  });

  it("isDirty tracks dto state", async () => {
      const dto: CentralReportViewModelDto = getCentralReportDto();
      const sut = new CentralReportLastPeriodEdit(new CentralPlanReportService(), new SignalrWrapper(), undefined);
      sut.reportid = 11;
      sut.centralReport = dto;
      ...
      sut.centralReport.associateMemberData.increased = 42;
      expect(sut.isDirty).toBe(true);
  ...
  it("cancelChanges calls expected service method and resets dirty flag", async () => {
      ...
      sut.setInitialData();
      expect(sut.isDirty).toBe(false);
      sut.centralReport.associateMemberData.increased = 55;
      expect(sut.isDirty).toBe(true);
  ...
  it("onUnitReportUpdated resets saving and calls expected method", () => {
      ...
      sut.centralReport.preliminaryMemberData.decreased = 34;
      sut.isSaving = true;
      expect(sut.isDirty).toBe(true);
      
      sut.onCentralReportUpdated(3);
  ...
  function getCentralReportDto(): CentralReportViewModelDto {
    return {
      ...
      function getExpectedReportData(dto: CentralReportViewModelDto): ReportLastPeriodUpdateData {
