...
import { UnitReportEdit } from "./unit-report-edit";
...
describe("unit report edit tests", () => {
    it("loadReport calls expected service method and sets data", async () => {
        ...
        const sut = new UnitReportEdit(svc, new SignalrWrapper(), undefined);
        sut.reportid = 3;
    ...
    it("save calls expected service method and sets flag", async () => {
        const dto: UnitReportViewModelDto = getUnitReportViewModelDto();  
        const expectedReportData = getExpectedReportData(dto);
        const svc = new UnitPlanReportService();
        spyOn(svc, "updateReport").and.returnValue(Promise.resolve(undefined));
        const sut = new UnitReportEdit(svc, new SignalrWrapper(), undefined);
        sut.reportid = 3;
        sut.unitReport = dto;
        ...
        expect(svc.updateReport).toHaveBeenCalled();
        expect(svc.updateReport).toHaveBeenCalledWith(5, 3, expectedReportData);
    ...
    it("save does nothin when not dirty", async () => {
      ...
      spyOn(unitPlanReportService, "updateReport").and.returnValue(Promise.resolve());
      const sut = new UnitReportEdit(unitPlanReportService, new SignalrWrapper(), undefined);
      sut.reportid = 3;
      sut.unitReport = dto;
      sut.setInitialData();
      ...
      //Assert
      expect(sut.isSaving).toBe(false);
      expect(unitPlanReportService.updateReport).not.toHaveBeenCalled();
    ...
    it("save calls expected service when dirty", async () => {
      ...
      spyOn(uniPlanReportService, "updateReport").and.returnValue(Promise.resolve());
      //const sut = new UnitReportEdit(uniPlanReportService, new SignalrWrapper(), this.router.navigate(`unit-report-edit/${3}`));
      const sut = new UnitReportEdit(uniPlanReportService, new SignalrWrapper(), undefined);
      sut.reportid = 3;
      sut.unitReport = dto;
      ...
      expect(sut.isSaving).toBe(true);
      expect(uniPlanReportService.updateReport).toHaveBeenCalledWith(5, 3, getExpectedReportData(dto));
      expect(sut.afterSave).toEqual([]);
  ...
  it("saveAndSubmit calls expected service when dirty and set expected afterSave", async () => {
    ...
    const expectedReportData = getExpectedReportData(sut.unitReport);
    expect(sut.isSaving).toBe(true);
    expect(unitPlanReportService.updateReport).toHaveBeenCalledWith(5, 3, expectedReportData);
    expect(sut.afterSave).toEqual([sut.submitReport]);
  });


  it("onUnitReportUpdated sets flag and calls afterSave", async () => {
    // Arrange
    const dto: UnitReportViewModelDto = getUnitReportViewModelDto();
    const unitPlanReportService = new UnitPlanReportService();
    ...
    expect(unitPlanReportService.submitReport).toHaveBeenCalledWith(5, 3);
  });  


  it("onUnitReportUpdated sets flag and does nothing if no afterSave", async () => {
    // Arrange
    const dto: UnitReportViewModelDto = getUnitReportViewModelDto();
    const unitPlanReportService = new UnitPlanReportService();
    spyOn(unitPlanReportService, "getReport").and.returnValue(Promise.resolve(dto));
    ...
    expect(unitPlanReportService.getReport).toHaveBeenCalled();
    expect(unitPlanReportService.submitReport).not.toHaveBeenCalled();
  });
  ...
  it("onUnitPlanUpdateFailed sets flag and does nothing if no afterSave", () => {
    ...
    sut.unitReport = dto;
    sut.setInitialData();
    sut.unitReport.preliminaryMemberData.increased = 25;
    expect(sut.isDirty).toBe(true);
    sut.afterSave = [];
  ...
  it("onUnitReportUpdateFailed sets flag and clears afterSave", () => {
    // Arrange
    const dto: UnitReportViewModelDto = getUnitReportViewModelDto();
    const svc = new UnitPlanReportService();
    spyOn(svc, "submitReport").and.returnValue(Promise.resolve());
    ...
    sut.unitReport = dto;
    sut.setInitialData();
    sut.unitReport.associateMemberData.increased = 35;
    expect(sut.isDirty).toBe(true);
    sut.afterSave = [sut.submitReport];
    ...
    it("isDirty tracks dto state", async () => {
        const dto: UnitReportViewModelDto = getUnitReportViewModelDto();
        const sut = new UnitReportEdit(new UnitPlanReportService(), new SignalrWrapper(), undefined);
        sut.reportid = 3;
        sut.unitReport = dto;
    ...
    it("submitReport calls expected service method and sets flag", async () => {
        const dto: UnitReportViewModelDto = getUnitReportViewModelDto();
        const svc = new UnitPlanReportService();
        spyOn(svc, "submitReport").and.returnValue(Promise.resolve());
        const sut = new UnitReportEdit(svc, new SignalrWrapper(), undefined);
        ...
        expect(svc.submitReport).toHaveBeenCalledWith(5, 3);
        expect(sut.isSubmitting).toBe(true);
    });

    it("onUnitReportSubmitted resets flag and calls expected router method", () => {
        const mockRouter = {
            navigate: ()=>{}
        };
        spyOn(mockRouter, "navigate");
        const sut = new UnitReportEdit(new UnitPlanReportService(), new SignalrWrapper(), mockRouter as any as AppRouter);
        sut.isSubmitting = true;

        sut.onUnitReportSubmitted(3);

        expect(sut.isSubmitting).toBe(false);
        expect(mockRouter.navigate).toHaveBeenCalledWith('unit-report-view/3');
    });

    it("onUnitReportSubmitFailed only resets submitting flag", () => {
        const mockRouter = {
            navigate: ()=>{}
        };
        spyOn(mockRouter, "navigate");
        ...
        sut.onUnitReportSubmitFailed({$values: ["error"]});
    ...
    function getUnitReportViewModelDto(): UnitReportViewModelDto {
      ...
      function getExpectedReportData(dto: UnitReportViewModelDto): ReportUpdateData{
        return new ReportUpdateData(
          ...
          makeMeetingProgramData(dto.workerMeetingProgramData),
          makeMeetingProgramData(dto.cmsMeetingProgramData),
          makeMeetingProgramData(dto.smMeetingProgramData),
          makeMeetingProgramData(dto.memberMeetingProgramData),
          makeMeetingProgramData(dto.dawahMeetingProgramData),
          makeMeetingProgramData(dto.stateLeaderMeetingProgramData),
          makeMeetingProgramData(dto.stateOutingMeetingProgramData),
          makeMeetingProgramData(dto.iftarMeetingProgramData),
          makeMeetingProgramData(dto.learningMeetingProgramData),
          makeMeetingProgramData(dto.socialDawahMeetingProgramData),
          makeMeetingProgramData(dto.dawahGroupMeetingProgramData),
          makeMeetingProgramData(dto.nextGMeetingProgramData),
          makeMeetingProgramData(dto.tafsirMeetingProgramData),
          makeMeetingProgramData(dto.unitMeetingProgramData),
          makeMeetingProgramData(dto.bbqMeetingProgramData),
          makeMeetingProgramData(dto.gatheringMeetingProgramData),
          makeMeetingProgramData(dto.familyVisitMeetingProgramData),
          makeMeetingProgramData(dto.eidReunionMeetingProgramData),
          makeMeetingProgramData(dto.otherMeetingProgramData),
          makeTeachingLearningProgramData(dto.groupStudyTeachingLearningProgramData),
          makeTeachingLearningProgramData(dto.studyCircleForAssociateMemberTeachingLearningProgramData),
          makeTeachingLearningProgramData(dto.studyCircleTeachingLearningProgramData),
          makeTeachingLearningProgramData(dto.practiceDarsTeachingLearningProgramData),
          makeTeachingLearningProgramData(dto.stateLearningCampTeachingLearningProgramData),
          makeTeachingLearningProgramData(dto.quranStudyTeachingLearningProgramData),
          makeTeachingLearningProgramData(dto.hadithTeachingLearningProgramData),
          makeTeachingLearningProgramData(dto.quranClassTeachingLearningProgramData),
          makeTeachingLearningProgramData(dto.weekendIslamicSchoolTeachingLearningProgramData),
          makeTeachingLearningProgramData(dto.memorizingAyatTeachingLearningProgramData),
          makeTeachingLearningProgramData(dto.memorizingHadithTeachingLearningProgramData),
          makeTeachingLearningProgramData(dto.memorizingDoaTeachingLearningProgramData),
          makeTeachingLearningProgramData(dto.stateLearningSessionTeachingLearningProgramData),
          makeTeachingLearningProgramData(dto.stateQiyamulLailTeachingLearningProgramData),
          makeTeachingLearningProgramData(dto.otherTeachingLearningProgramData),
          makeSocialWelfareReportData(dto.qardeHasanaSocialWelfareData),
          makeSocialWelfareReportData(dto.patientVisitSocialWelfareData),
          makeSocialWelfareReportData(dto.socialVisitSocialWelfareData),
          makeSocialWelfareReportData(dto.transportSocialWelfareData),
          makeSocialWelfareReportData(dto.shiftingSocialWelfareData),
          makeSocialWelfareReportData(dto.shoppingSocialWelfareData),
          makeSocialWelfareReportData(dto.foodDistributionSocialWelfareData),
          makeSocialWelfareReportData(dto.cleanUpAustraliaSocialWelfareData),
          makeSocialWelfareReportData(dto.otherSocialWelfareData),
          makerMaterialData(dto.bookSaleMaterialData),
          makerMaterialData(dto.bookDistributionMaterialData),
          makerMaterialData(dto.vhsSaleMaterialData),
          makerMaterialData(dto.vhsDistributionMaterialData),
          makerMaterialData(dto.emailDistributionMaterialData),
          makerMaterialData(dto.ipdcLeafletDistributionMaterialData),
          makerMaterialData(dto.otherSaleMaterialData),
          makerMaterialData(dto.otherDistributionMaterialData),
          makeLibraryStockData(dto.bookLibraryStockData),
          makeLibraryStockData(dto.vhsLibraryStockData),
          makeLibraryStockData(dto.otherLibraryStockData),
          makeFinancePlanReportData(dto.baitulMalFinanceData),
          makeFinancePlanReportData(dto.aDayMasjidProjectFinanceData),
          makeFinancePlanReportData(dto.masjidTableBankFinanceData),
          dto.comment);
      }

      function makeMeetingProgramData(original: MeetingProgramData): MeetingProgramReportData {
      ...
      function makeTeachingLearningProgramData(original: TeachingLearningProgramData): TeachingLearningProgramData {
      ...
      function makeSocialWelfareReportData(original: SocialWelfareData): SocialWelfareReportData {
          return new SocialWelfareReportData(original.actual, original.comment);
      }

      function makerMaterialData(original: MaterialData): MaterialData {
        return new MaterialData(original.target, original.dateAndAction, original.actual, original.comment);
      }

      function makeLibraryStockData(original: LibraryStockData): LibraryStockData {
      ...
      function makeFinancePlanReportData(original: FinanceData): FinanceReportData {
