...
{
    public class CentralReportViewModelMap2 : ClassMap<CentralReportViewModel>
    {
        public CentralReportViewModelMap2()
        {
            Not.LazyLoad();
            ReadOnly();

            Table("Report");
            Where("IsDeleted = 0 AND OrganizationOrganizationType = 5");
            Id(x => x.Id);
            Map(x => x.Description);
