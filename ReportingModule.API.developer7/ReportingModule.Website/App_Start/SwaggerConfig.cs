...
namespace ReportingModule.Website
{
    public class SwaggerConfig
    {
        ...
        {
            var containingAssembly = typeof(UnitReportViewModel).Assembly;

            GlobalConfiguration.Configuration
                .EnableSwagger(c =>
                {
                    c.SingleApiVersion("v1", "Reporting Api");

                    c.IncludeXmlComments($@"{System.AppDomain.CurrentDomain.BaseDirectory}\bin\ReportingModule.Website.xml");
    ...
    public class FilterPathItems : IDocumentFilter
    {
        public void Apply(SwaggerDocument swaggerDoc, SchemaRegistry schemaRegistry, IApiExplorer apiExplorer)
        {
            swaggerDoc.paths = swaggerDoc.paths
                .Where(entry => entry.Key.StartsWith("/reporting"))
