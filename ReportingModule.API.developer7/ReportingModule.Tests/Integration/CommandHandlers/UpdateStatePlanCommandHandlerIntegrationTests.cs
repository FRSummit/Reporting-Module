...
namespace ReportingModule.Tests.Integration.CommandHandlers
{
    ...
    public class UpdateStatePlanCommandHandlerIntegrationTests
    {
        [SetUp]
        public void Setup()
        {
        ...
        public async Task Handle_SavesPlan(ReportingFrequency reportingFrequency )
        {
            ...
            var testParams = Endpoint.ArrangeOnSqlSession(AssemblySetupFixture.EndpointTestContainer,
                s =>
                {
                    ...
                    var expectedEvt = Test.CreateInstance<IStatePlanUpdated>(e =>
                    {
                    ...
                    return new
                    {
                        Cmd = new UpdateStatePlanCommand(report.Id,
                            planData),
            ...
            var context = await Endpoint.Act<UpdateStatePlanCommandHandler>(AssemblySetupFixture.EndpointTestContainer,
                (h, ctx) =>
                    {
                        ctx.SetUsernameOnHeader(testParams.username);
                        return h.Handle(testParams.Cmd, ctx);
                    });
            var evt = context.ExpectPublish<IStatePlanUpdated>();
