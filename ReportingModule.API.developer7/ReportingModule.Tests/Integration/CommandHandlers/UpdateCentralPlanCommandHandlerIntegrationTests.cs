...
namespace ReportingModule.Tests.Integration.CommandHandlers
{
    ...
    public class UpdateCentralPlanCommandHandlerIntegrationTests
    {
        [SetUp]
        public void Setup()
        {
        ...
        public async Task Handle_SavesPlan(ReportingFrequency reportingFrequency)
        {
            ...
            var testParams = Endpoint.ArrangeOnSqlSession(AssemblySetupFixture.EndpointTestContainer,
                s =>
                {
                    ...
                    var expectedEvt = Test.CreateInstance<ICentralPlanUpdated>(e =>
                    {
                    ...
                    return new
                    {
                        Cmd = new UpdateCentralPlanCommand(report.Id,
                            planData),
            ...
            var context = await Endpoint.Act<UpdateCentralPlanCommandHandler>(AssemblySetupFixture.EndpointTestContainer,
                    (h, ctx) =>
                    {
                        ctx.SetUsernameOnHeader(testParams.username);
                        return h.Handle(testParams.Cmd, ctx);
                    });
            var evt = context.ExpectPublish<ICentralPlanUpdated>();
