...
namespace ReportingModule.Tests.Integration.CommandHandlers
{
    ...
    public class UnSubmitReportCommandHandlerIntegrationTests
    {
        [SetUp]
        public void Setup()
        {
        ...
        public async Task Handle_UnSubmitPromotedPlan(OrganizationType organizationType, ReportingFrequency reportingFrequency)
        {
            ...
            var testParams = Endpoint.ArrangeOnSqlSession(AssemblySetupFixture.EndpointTestContainer,
                s =>
                {
                    ...
                    var expectedEvt = Test.CreateInstance<IReportUnSubmitted>(e =>
                    {
                    ...
                    return new
                    {
                        Cmd = new UnSubmitReportCommand(report.Id),
            ...
            var context = await Endpoint.Act<UnSubmitReportCommandHandler>(AssemblySetupFixture.EndpointTestContainer,
                (h, ctx) =>
                    {
                        ctx.SetUsernameOnHeader(testParams.username);
                        return h.Handle(testParams.Cmd, ctx);
                    });
            var evt = context.ExpectPublish<IReportUnSubmitted>();
            ...
            Endpoint.AssertOnSqlSessionThat(AssemblySetupFixture.EndpointTestContainer,
                s =>
                {
                    ...
                    report.ReportStatus.Should().Be(ReportStatus.Draft);
        ...
        [TestCase(OrganizationType.Central, ReportingFrequency.Quarterly)]
        [TestCase(OrganizationType.Central, ReportingFrequency.Yearly)]
        public async Task Handle_UnSubmitReport(OrganizationType organizationType, ReportingFrequency reportingFrequency)
        {
            DateTimeDbTestExtensions.SetUtcNowToRandomDate();
