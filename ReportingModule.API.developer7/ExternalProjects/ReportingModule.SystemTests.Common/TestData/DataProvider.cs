...
namespace ReportingModule.SystemTests.Common.TestData
{
	...
	public static class DataProvider
	{
        ...
        public static Money CreateAud()
        {
            return new Money(Random.Next(10, 5000), Currency.AUD);
        }

        public static Money CreateMoney(Currency currency)
        {
            return new Money(Random.Next(10, 5000), currency);
        }
	    ...
	    private static readonly IDictionary<Type, Func<object>> DefaultProviders = new Dictionary<Type, Func<object>>
	    {
	        ...
	        {typeof(DateRange), RandomDateRange},
	        {typeof(TimeSpan), RandomTimeSpan},
	        {typeof(DateTimeOffset), RandomDateTimeOffset},
	        {typeof(DateTimeRange), RandomDateTimeRange},
	        {typeof(int), NextInt},
	        {typeof(EntityReference), NextEntityReference},
	        {typeof(UserReference), RandomUserRef},
	        {typeof(string), NextString},
	        {typeof(char), NextChar},
	        {typeof(Money), RandomMoney},
	        {typeof(bool), () => false},
        ...
        private static object RandomDateRange()
		{
            var from = RandomDate(DateTimeKind.Unspecified).Date;
		    return new DateRange(from, from.AddDays(Get<int>()));
		}

		private static object RandomDateTimeRange()
		{
			var from = RandomDate(DateTimeKind.Unspecified);
			var to = from.AddDays(_nextInt++);
	    ...
	    public static object RandomUserRef()
	    {
            int id = _nextInt++;
	        return new UserReference(id, Get<string>(), Get<string>(), GetValidEmail());
	    }
		...
		private static object RandomMoney()
		{
			var randomCurrency = RandomEnum<Currency>();
			return new Money(Random.Next(10, 5000), randomCurrency);
		}
		...
		private static object NextEntityReference()
		{
			int id = _nextInt++;
			return new EntityReference(id, $"EntityRef {id}: from DataProvider");
		}
	    ...
	    public static EntityReference GetErWithStringOfLength(int length)
	    {
	        return new EntityReference((int)NextInt(), GetStringOfLength(length));
	    }
	}
}